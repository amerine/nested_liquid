...
module Liquid
  ...
  class Context
    ...
    def variable(markup)
      ...
      if Liquid.allowed_namespaces.to_a.include? parts.first
        #Liquid 2.2 uses environments
        if @environments
          @environments.each do |environment|
            return render_nested_liquid(environment[first_part].send(parts.first).send(parts.second), self) if environment.has_key? first_part
          end
        else
          # Liquid < 2.2 uses scopes
          @scopes.each do |scope|
            return render_nested_liquid(scope[first_part].send(parts.first).send(parts.second), self) if scope.has_key? first_part
          end
        end
      else
